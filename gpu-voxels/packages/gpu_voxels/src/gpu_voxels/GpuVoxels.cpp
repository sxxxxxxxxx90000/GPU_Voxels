...
bool GpuVoxels::addRobot(const std::string &robot_name, const std::string &path_to_urdf_file, const bool use_model_path)
{
  ...
  // robot::UrdfRobot urdf_robot(m_voxel_side_length, path_to_urdf_file, use_model_path);
  RobotInterfaceSharedPtr ri_ptr(new robot::UrdfRobot(m_voxel_side_length, path_to_urdf_file, use_model_path));
  std::pair<std::string, RobotInterfaceSharedPtr> pair_tmp = std::make_pair(robot_name, ri_ptr);
  m_managed_robots.insert(pair_tmp);
